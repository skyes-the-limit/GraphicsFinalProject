...
const createProgramFromScripts = (gl, vertexShaderElementId,
...
let shapes = [
  ...
  },
  {
    ...
    color: {
      red: GREEN_RGB.red,
      green: GREEN_RGB.blue,
      blue: GREEN_RGB.green
    }
  },
...
const renderCircle = (circle) => {
  ...
  for (let radians = 0; radians < 2 * Math.PI; radians += Math.PI / 20) {
    ...
    pointArray.push(Math.cos(radians - Math.PI / 20) * circle.dimensions.width / 2
        + circle.position.x);
    pointArray.push((Math.sin(radians - Math.PI / 20) * circle.dimensions.height / 2)
        + circle.position.y);
